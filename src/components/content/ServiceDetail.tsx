...
import { industries, type ContentPage } from "@/lib/site";
import { SERVICE_EYEBROW, SERVICE_VALUE, SERVICE_EXPERTISE, SERVICE_WHY, SERVICE_HOW } from "@/lib/serviceDetail";
import { PageHero } from "@/components/content/PageTemplate";
import { EnquiryForm } from "@/components/content/EnquiryForm";

export function ServiceDetail({ page, siblings }: { page: Omit<ContentPage, "icon">; siblings: { title: string; href: string }[] }) {
  const value = SERVICE_VALUE[page.slug] ?? [];
  const expertise = SERVICE_EXPERTISE[page.slug] ?? [];
  const eyebrow = SERVICE_EYEBROW[page.slug] ?? "XR ENGINEERING SERVICES";
  const faqSchema = {
  ...
  return (
    <>
      ...
      <PageHero
        ...
        crumbs={[{ label: "Services", href: "/digital-product-engineering" }]}
      />
      ...
      <section className="py-14 md:py-20 bg-white">
        <div className="wrap grid lg:grid-cols-[1fr_1.5fr] gap-10">
          <div>
            ...
            <p className="mt-4 text-lg text-[#626a6d]">Collaboration, scalability and business alignment — from first workshop to production support.</p>
      ...
      <section className="py-14 md:py-20 bg-[#F4F8FA]">
        <div className="wrap">
          ...
          <div className="mt-6 rounded-[20px] border border-[#1F2A2E]/10 bg-white p-6">
            <p className="text-sm font-bold text-[#626a6d]">Also in Digital Product Engineering:</p>
            <div className="mt-3 flex flex-wrap gap-2">
              {siblings.map((s) => (
                <Link key={s.href} href={s.href} className="rounded-full border border-[#1F2A2E]/15 px-4 py-2 text-sm font-bold hover:bg-[#1F2A2E] hover:text-white transition">{s.title}</Link>
              ))}
            </div>
          </div>
        </div>
